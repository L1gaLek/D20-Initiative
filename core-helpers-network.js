...
// Fetch latest room_state row (source of truth for low-frequency state)
...
function buildRoomStateWritePayload(roomId, nextState, latestRoomState) {
...
// Apply initiative updates for owned players with compare-and-swap retry to avoid lost writes.
...
async function applyInitiativeAtomic(roomId, myUserId, updates) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    for (const u of updArr) {
      ...
      const wantTotal = Number(u.total);
      if (p.hasRolledInitiative && Number(p.initiative) === wantTotal) continue;
      if (p.hasRolledInitiative && Number(p.initiative) !== wantTotal) continue;

      p.initiative = wantTotal;
    ...
    const { payload, syncedState } = buildRoomStateWritePayload(roomId, next, latest);
    let query = sbClient
      .from("room_state")
      .update({
        phase: payload.phase,
...
async function upsertRoomState(roomId, nextState) {
  ...
  const { payload, syncedState } = buildRoomStateWritePayload(roomId, nextState, latestRoomState);
...
function formatDiceEventLogText(ev) {
  try {
    if (!ev) return '';
    const who = String(ev.fromName || '').trim() || 'Игрок';
    const kind = String(ev.kindText || '').trim() || (ev.sides ? `d${Number(ev.sides) || ''}` : 'Бросок');
    ...
    const body = rolls ? `${rolls}${bonusTxt}${totalTxt}` : String(ev.total ?? '');
...
async function insertDiceEvent(roomId, ev) {
  try {
    ...
    try {
      const line = formatDiceEventLogText({
