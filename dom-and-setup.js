...
function formatDiceEventLogTextLocal(event) {
  try {
    if (!event) return '';
    const who = String(event.fromName || '').trim() || 'Игрок';
    const kind = String(event.kindText || '').trim() || (event.sides ? `d${Number(event.sides) || ''}` : 'Бросок');
    const rolls = Array.isArray(event.rolls) ? event.rolls.map(n => Number(n) || 0).join(',') : '';
    const bonusNum = Number(event.bonus) || 0;
    const bonusTxt = bonusNum === 0 ? '' : (bonusNum > 0 ? `+${bonusNum}` : String(bonusNum));
    const totalTxt = (event.total === null || typeof event.total === 'undefined') ? '' : ` = ${event.total}`;
    const critTxt = event.crit === 'crit-success' ? ' (КРИТ)' : (event.crit === 'crit-fail' ? ' (ПРОВАЛ)' : '');
    const body = rolls ? `${rolls}${bonusTxt}${totalTxt}` : String(event.total ?? '');
...
async function broadcastDiceEventOnly(event) {
  ...
  // When Supabase Realtime is OFF, mirror both the dice event and the action-log row through VPS WS.
  try {
    if (!event) return;
  ...
  try {
    const line = formatDiceEventLogTextLocal(event);
    if (line) handleMessage({ type: 'logRow', row: { text: line, created_at: new Date().toISOString() } });
  } catch {}

  try {
    if (!USE_SUPABASE_REALTIME && currentRoomId && typeof sendWsEnvelope === 'function' && event) {
      sendWsEnvelope({ type: 'diceEvent', roomId: currentRoomId, event }, { optimisticApplied: true });
      const line = formatDiceEventLogTextLocal(event);
      if (line) {
        sendWsEnvelope({
          type: 'logRow',
          roomId: currentRoomId,
          row: { text: line, created_at: new Date().toISOString() }
